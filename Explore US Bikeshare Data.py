...
def get_filters():
    ...
    while day not in days:
        day = input('ENTER DAY (e.g., monday, tuesday, ... sunday or all): ').lower()
...
def load_data(city, month, day):
    ...
    required_columns = ['Start Time', 'End Time', 'Trip Duration', 'Start Station', 'End Station']
...
def raw_data(df):
    """Displays the data due to filtration, 5 rows at a time."""
    x = 0
    while True:
        more_data = input('Would you like to see 5 more rows of data? Enter yes or no: ').lower()
        if more_data == 'no':
            break
        elif more_data == 'yes':
            print(df.iloc[x:x+5])
